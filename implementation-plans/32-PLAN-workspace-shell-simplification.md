...
**Relevant files**
- `src/lib/components/WorkspaceShell.svelte` — central shell layout, topbar, sidebar section composition, bottom-panel removal, and prop threading.
- `src/App.svelte` — state orchestration, virtual-tab/file-opening flow, pipeline status state, and callback cleanup.
- `src/lib/components/SidebarPromptBlocks.svelte` — reorder Registered vs Available templates and add local collapsed subsections.
- `src/lib/components/SidebarModels.svelte` — convert from sidebar management UI to file-list navigation behavior.
- `src/lib/components/SidebarGlobalVariables.svelte` — replace inline editing with virtual-file navigation entry behavior.
- `src/lib/components/SidebarWorkspaceVariables.svelte` — replace inline editing with file-like navigation behavior for workspace/project variables.
- `src/lib/components/ValidationPanel.svelte` — retire or repurpose bottom-panel-only execution/history UI.
- `src/lib/components/InspectorPanel.svelte` — absorb any remaining validation/run detail that should live in the inspector after bottom-panel removal.
- `src/lib/components/AssetViewer.svelte` — preserve prompt preview/editor behavior after shell simplification.
- `src/lib/components/SidebarPipelines.svelte` and `src/lib/components/PipelineEditorTab.svelte` — reference current live-progress UI and last-run semantics while moving topbar status.
- `src/lib/types/project.ts` — confirm whether any virtual-asset or topbar status types need extension.
- `src/App.test.ts` — update shell-level behavior tests.
...
- Excluded scope: the user-edited Ghost Draft prompt files under `Sample Projects/NPE Ghost Draft/prompts/`.
