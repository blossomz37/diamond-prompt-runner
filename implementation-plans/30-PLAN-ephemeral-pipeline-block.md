...
**Steps**
1. Backend execution contract: extend the pipeline execution call chain to accept an optional selected block ID list. This means updating the Tauri command in `/Users/carlo/diamond-runner/src-tauri/src/main.rs`, the public execution function in `/Users/carlo/diamond-runner/src-tauri/src/project_store/execution.rs`, and the transport-backed helper in the same file. This step blocks the frontend wiring because the UI needs a stable runtime contract.
2. Backend filtering behavior: in `/Users/carlo/diamond-runner/src-tauri/src/project_store/execution.rs`, derive an ordered `blocks_to_run` list from the saved pipeline’s `ordered_blocks`, filtered by the selected IDs while preserving canonical pipeline order. Validate that the selection is not empty and that selected IDs belong to the pipeline. Use this filtered list for execution, progress events, and resume indexing.
...
4. Bridge update: extend `/Users/carlo/diamond-runner/src/lib/tauri.ts` so `executePipeline` accepts `selectedBlockIds?: string[]` and forwards them to the Tauri command. This can run in parallel with step 2 once the backend contract is decided.
5. Pipeline editor UI: update `/Users/carlo/diamond-runner/src/lib/components/PipelineEditorTab.svelte` to add an ephemeral run mode for existing pipelines. Recommended UX: default “Run all” behavior plus an opt-in “Run selected” mode that reveals checkboxes beside the read-only step list. All blocks should start selected when entering this mode.
6. Run action wiring: update `/Users/carlo/diamond-runner/src/lib/components/WorkspaceShell.svelte` and `/Users/carlo/diamond-runner/src/App.svelte` so the run callback can carry selected block IDs from the editor to `handleRunPipeline`, and from there to the Tauri bridge. Keep selection state local to the editor and reset it when switching pipelines.
7. Guardrails in the UI: in `/Users/carlo/diamond-runner/src/lib/components/PipelineEditorTab.svelte`, disable the run action when no blocks are selected, surface a concise message when resume is incompatible with the current selection, and keep existing dirty-prompt guards in `/Users/carlo/diamond-runner/src/App.svelte` unchanged so subset runs still require saved `.tera` files.
...
9. Frontend verification: add or extend the relevant UI test coverage in `/Users/carlo/diamond-runner/src/App.test.ts` only if there is already a practical mocking path for the pipeline editor flow; otherwise rely on focused manual verification for the first slice and keep automated coverage backend-heavy.
...
**Relevant files**
- `/Users/carlo/diamond-runner/src-tauri/src/main.rs` — extend the `execute_pipeline` Tauri command signature and forward selected block IDs.
- `/Users/carlo/diamond-runner/src-tauri/src/project_store/execution.rs` — filter `ordered_blocks` into a runtime subset, validate selection, and keep progress/resume logic coherent.
- `/Users/carlo/diamond-runner/src/lib/tauri.ts` — extend the `executePipeline` bridge wrapper.
- `/Users/carlo/diamond-runner/src/App.svelte` — thread optional selected block IDs through `handleRunPipeline` while preserving existing dirty-file checks and run history refresh.
- `/Users/carlo/diamond-runner/src/lib/components/WorkspaceShell.svelte` — pass the enriched run callback into `PipelineEditorTab`.
- `/Users/carlo/diamond-runner/src/lib/components/PipelineEditorTab.svelte` — add the per-run selection UI, local selection state, and run-mode behavior.
- `/Users/carlo/diamond-runner/src/App.test.ts` — optional UI-level coverage if the editor flow is practical to mock.
