 ...
 * Map the input path, output path, heading levels, and downstream retrieval
 * needs to the user's actual project first.
 *
 * Purpose:
 * Convert heading-based markdown into nested bracket-boundary sections.
 *
 * Usage:
 *   node normalize-markdown-boundaries.mjs input.md [output.md] [--exact-level=N] [--levels=2,3] [--preserve-heading]
 */
import fs from 'fs';
...
function shouldWrapLevel(level, exactLevel, levelSet) {
  if (levelSet) return levelSet.has(level);
  if (exactLevel !== null) return level === exactLevel;
  return true;
}
...
function main() {
  ...
  if (!inputPath) {
    console.error('Usage: node normalize-markdown-boundaries.mjs input.md [output.md] [--exact-level=N] [--levels=2,3] [--preserve-heading]');
    process.exit(1);
  }
